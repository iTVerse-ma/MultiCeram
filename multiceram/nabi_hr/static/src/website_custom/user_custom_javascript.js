/* @odoo-module */
// JavaScript personnalisé saisi dans l'éditeur du site mc2 (ir.attachment 1566, bundle web.assets_frontend_lazy),
// repris tel quel ; seules adaptations Odoo 19 : import de markup et appels rpc().
...
// Attendance Widget
// ##############################
publicWidget.registry.ExportCSV = publicWidget.Widget.extend({
  selector: ".o_portal_wrap",   // wrapper attendu dans ton template
  

    // seuls les clics sur actions restent dans events (les inputs sont attachés dans start)
    events: {
        "click #exportCSV"              : "tableToCSV",
        
    },
  
  
  async tableToCSV(tableId="myTable", filename = "export.csv") {
    console.log("sssss");
    const table = this.el.querySelector('[data-to-export="true"]');
    let csv = [];
...
publicWidget.registry.AttendancePortal = publicWidget.Widget.extend({
    selector: "#attendance-portalss",   // wrapper attendu dans ton template
    ...
    // Helpers
    // ------------------------------
    async _loadPage(page = 1, append = false) {
            ...
            if (result && result.portal_data && result.portal_data.length > 0 && result.portal_data[0].length > 0) {
                result.portal_data[0].forEach(record => {
                    ...
                    const row = xml`<tr data-employee="${employeeName.toLowerCase()}" data-last-action="${lastAction}">
                        ...
                        <td>${employeeName}</td>
                        <td class="punches">xxxxx
                            ${(record.punches || []).map(
                                punch => `<span class="card badge bg-primary ms-1">
                                    ${(!punch.terminal_alias ? 'M' : 'A')} ${punch.punch_time.split(" ")[1] || ''}
    ...
    _applyFilter() {
        ...
        rows.forEach(row => {
            ...
            if (search && !emp.includes(search)) show = false;
    ...
    async _onAddPunch(ev) {
        ...
        const formHtml = `<div class="text-nowrap">
            <form id="manualPunchForm" >
                ...
                <input type="datetime-local" name="punch_time" class="form-control form-control-sm" required="1" value="${nowStr}"/>
    ...
    async _onAddLeave(ev) {
        ...
        const formHtml = `<div class="text-nowrap">
            <form id="leaveForm">
                ...
                <input type="datetime-local" name="start_time" class="form-control form-control-sm" required value="${nowStr}"/>
                <label class="fw-bold small">End</label>
                <input type="datetime-local" name="end_time" class="form-control form-control-sm" required value="${nowStr}"/>
                <label class="fw-bold small">Reason</label>
                <input type="text" name="apply_reason" class="form-control form-control-sm" placeholder="Reason (optional)"/>
            </form></div>`;
