...
publicWidget.registry.hsValidation = publicWidget.Widget.extend({
    
    selector: '#hs-validation',  // wrapper around your filter and table

    events: {
        "change #select_all": "_onSelectAll",
        "change .row-select": "_onRowSelect",
        "click [data-action=bulk_delete]": "_onBulkDelete",
        "click [data-action=bulk_export]": "_onBulkExport",
    ...
    start() {
        this.$bulkButtons = document.querySelectorAll("[data-action^='bulk_']");
    ...
    _onSelectAll(ev) {
        ...
        this.el.querySelectorAll("tbody tr:not([hidden]) .row-select")
    ...
    // ✅ Single row select
    _onRowSelect() {
        const rows = this.el.querySelectorAll("tbody tr:not([hidden])  .row-select");
        const checked = [...rows].filter(cb => cb.checked).length;

        this.el.querySelector("#select_all").checked =
            checked === rows.length && rows.length > 0;
    ...
    _updateBulkButtons() {
        const selected = this._getSelectedIds().length > 0;
        this.$bulkButtons.forEach(btn => btn.disabled = !selected);
    },

    // 📥 Collect selected row IDs
    _getSelectedIds() {
        return [...this.el.querySelectorAll("tbody tr .row-select:checked")]
    ...
    _onBulkDelete() {
        ...
        if (!confirm(`Supprimer ${ids.length} lignes ?`)) return;

        console.log("DELETE IDS:", ids);

        // 👉 Exemple RPC Odoo
        /*
        rpc("/my/bulk/delete", { ids })
    ...
    async _tableToXLSX(table, filename="export.xlsx") {
    ...
    async _onDelete(ev) {
        ...
        this.dialog.add(ConfirmationDialog, {
            ...
            confirm: async () => {
                const result = await rpc(`/my/attendance/delete/${id}`, { csrf_token: odoo.csrf_token });
                this.dialog.add(ConfirmationDialog, {
    ...
    async _onCopy(ev) {
        ...
        this.dialog.add(ConfirmationDialog, {
            ...
            confirm: async () => {
                const result = await rpc(`/my/attendance/copy/${id}`, { csrf_token: odoo.csrf_token });
                this.dialog.add(ConfirmationDialog, {
                    title: result.status === "success" ? "✅ Success" : "❌ Error",
                    body: result.message,
                });
    ...
    async _onAddJourRepos(ev){
        ...
        const result = await rpc("/my/attendance/add_jour_repos", payload);
        this.dialog.add(ConfirmationDialog, {
    ...
    async _onAddPunch(ev) {
            ...
            this.dialog.add(ConfirmationDialog, {
                ...
                confirm: async () => {
                    ...
                    const result = await rpc("/my/attendance/add_punch", payload);
                    this.dialog.add(ConfirmationDialog, {
                        title: result.status === "success" ? "✅ Created" : "❌ Error",
                        body: result.message,
                        confirm: () => {
        ...
        async _onAddLeave(ev) {
            ...
            this.dialog.add(ConfirmationDialog, {
                ...
                confirm: async (eev) => {
                    ...
                    const result = await rpc("/my/leaves/add_leaves", payload);
                    this.dialog.add(ConfirmationDialog, {
                        title: result.status === "success" ? "✅ Created" : "❌ Error",
                        body: result.message,
                    });
    ...
    _onFilter(ev) {
        ...
        const Departments = Array.from(this.el.querySelector("#filter_department")?.selectedOptions).map(opt=>opt.value) || false;
        ...
        form.method = 'POST';
        form.action = '/my/HeureSupp/Validation';
        ...
        addInput('department_ids', Departments);
