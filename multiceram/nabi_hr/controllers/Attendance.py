...
from .odoo19 import odoo18_bound
...
class AttendancePortal(http.Controller):
    ...
    def portal_my_report(self, start_day=None, end_day=None, emp_id=None,date=None ,**kw):
        ...
        domain = [('duplicate','=',False),('to_delete','=',False),('punch_time','>=',odoo18_bound(start_day, '>=')),('punch_time','<=',odoo18_bound(end_day, '<='))]
        try:
            emp_id = int(emp_id)
    ...
    @http.route('/my/transaction/copy/<int:transaction_id>', type='jsonrpc', auth='user', website=True, methods=['POST','GET'],csrf=False)
    ...
    def portal_get_punch_stacked(self, start_day = None, end_day=None, emp_id=0,terminal_ids=[],**kw):
        ...
        domain = [('duplicate','=',False),('to_delete','=',False),('punch_time','>=',odoo18_bound(start_day, '>=')),('punch_time','<=',odoo18_bound(end_day, '<=')),('emp_code','=',emp_id)]
        ...
        jf =  request.env['resource.calendar.leaves'].search([('resource_id','=',False),
                                                              '|','&',('date_from','>=',odoo18_bound(start_day, '>=')),('date_from','<=',odoo18_bound(end_day, '<=')),
                                                              '&',('date_to','>=',odoo18_bound(start_day, '>=')),('date_to','<=',odoo18_bound(end_day, '<='))])
    ...
    def portal_get_punches(self, start_day = None, end_day=None, emp_id=0,terminal_ids=None,**kw):
        ...
        domain = [('duplicate','=',False),('to_delete','=',False),('punch_time','>=',odoo18_bound(start_day, '>=')),('punch_time','<=',odoo18_bound(end_day, '<=')),('employee_id','=',emp_id)]
    ...
    @http.route(['/my/attendance/add_jour_repos'], type='jsonrpc', auth='user', website=True, methods=['POST'],csrf=False)
    ...
    @http.route(['/my/attendance/add_punch'], type='jsonrpc', auth='user', website=True, methods=['POST'],csrf=False)
    ...
    @http.route(['/my/attendance/delete/<int:emp_id>'], type='jsonrpc', auth='user', website=True, methods=['POST'])
    ...
    # @http.route(['/my/attendance/json/<string:start_day>','/my/attendance/json'], type='jsonrpc', auth='user',methods=['POST'], website=True,csrf=False)
    ...
    #     domain = [('punch_time','>=',odoo18_bound(start_day, '>=')),('punch_time','<=',odoo18_bound(end_day, '<=')),('emp_code','!=',False)]
    #     # employees = request.env['hr.employee'].sudo().search(domain)
    ...
    def portal_attendance_json(self, start_day=None, end_day=None, page=1, page_size=10,filters={}, **kw):
        ...
        # --------------------------
        domain = [
            ('punch_time', '>=', odoo18_bound(start_day, '>=')),
            ('punch_time', '<', odoo18_bound(end_day + timedelta(days=1), '<')),  # overnight
            ('emp_code', '!=', False),
    ...
    def portal_get_anomalie(self, start_day: date = None, end_day: date = None, emp_id : int = 0,terminal_ids :list =[],**kw):
        ...
        domain = [('punch_time','>=',odoo18_bound(start_day, '>='))] 
        if end_day:
            domain+= [('punch_time','<=',odoo18_bound(end_day, '<='))] 
        else:
            domain+= [('punch_time','<',odoo18_bound(start_day+timedelta(days=1), '<'))] 
