...
with sync_playwright() as p:
    ...
    for label, (base, db) in SITES.items():
        ...
        page.screenshot(path='%s/v2_home_anonymous_%s.png' % (OUT, label))
        page.request.post(base + '/web/session/authenticate', data={"jsonrpc": "2.0", "method": "call", "params": {"db": db, "login": "admin", "password": "admin"}})
        page.goto(base + '/', wait_until='networkidle'); page.wait_for_timeout(1500)
        page.screenshot(path='%s/v2_home_logged_%s.png' % (OUT, label))
        page.goto(base + '/my/attendance/get_anomalie?start_day=2025-10-15', wait_until='networkidle', timeout=120000); page.wait_for_timeout(1500)
        page.screenshot(path='%s/v2_anomalies_%s.png' % (OUT, label))
        ctx.close()
    browser.close()
print("captured")
