...
class AttendancePortal(http.Controller):
    ...
    @http.route(['/my/attendance/get_punches'], type='http', auth='user', website=True, methods=['POST'],csrf=False)
    def portal_get_punches(self, start_day = None, end_day=None, emp_id=0,terminal_ids=None,**kw):
        ...
        domain = [('punch_time','>=',start_day),('punch_time','<=',end_day),('employee_id','=',emp_id)]
        ...
        # limit=  page_size
        punches         = request.env['zk.transaction'].search(domain).sorted(key=lambda x:x.punch_time)
    ...
    @http.route(['/my/attendance/json/<string:start_day>', '/my/attendance/json'],
                type='json', auth='user', methods=['POST'], website=True, csrf=False)
    def portal_attendance_json(self, start_day=None, end_day=None, page=1, page_size=10, **kw):
        """Return filtered attendance punches for portal display"""
        ...
        # Extract filters from POST data (kw)
        search_text         = kw.get('search', '').strip()
        terminal_filter     = kw.get('terminal', '').strip()
        date_from           = kw.get('from')
        date_to             = kw.get('to')
        include_doors       = kw.get('include_doors', True)
        include_restaurant  = kw.get('include_restaurant', True)
        ...
        include_doors = str2bool(include_doors)
        include_restaurant = str2bool(include_restaurant)
        ...
        # Filter by terminal
        if terminal_filter:
            domain.append(('terminal_alias', 'ilike', terminal_filter))

        # Fetch transactions
        offset = (int(page) - 1) * int(page_size)
        limit = int(page_size)

        punches = request.env['zk.transaction'].sudo().search(domain, offset=offset, limit=limit)
        punches = punches.sorted(key=lambda x: ((x.employee_id.barcode or '').zfill(6)))

        # Filter by usage type
        usage_domain = []
        if not include_doors:
            usage_domain.append(('usage', '!=', 'p'))
        if not include_restaurant:
            usage_domain.append(('usage', '!=', 'r'))
        if usage_domain:
            punches = punches.filtered_domain(usage_domain)
        ...
        for punch in punches:
            punch_day = fields.Date.to_string(fields.Date.from_string(str(punch.punch_time))[:10])
            employee = punch.employee_id
            if not employee:
                continue

            emp_key = employee.id
            portal_data.setdefault(punch_day, {})
            portal_data[punch_day].setdefault(emp_key, {
                'emp_id': punch.emp,
                'employee_id': employee.read(['barcode', 'name'])[0],
                'punches': [],
                'entrance_punches': [],
                'restaurant': [],
                'exceptions': [],
            ...
            # Link punches by TID
            portal_data[punch_day][emp_key]['punches'] += [p for p in main_punches if p['tid'] == punch['tid']]
            portal_data[punch_day][emp_key]['entrance_punches'] += [p for p in entrance if p['tid'] == punch['tid']]
            portal_data[punch_day][emp_key]['restaurant'] += [p for p in restaurant if p['tid'] == punch['tid']][:1]

        # Detect anomalies
        new_portal_data = portal_data.copy() or {}
        for day, emp_dict in portal_data.items():
            for emp_id, data in emp_dict.items():
                day_punches = data['punches']
                day_entrance = data['entrance_punches']
                day_restaurant = data['restaurant']

                exceptions, clean_punches = self._detect_exceptions(day_punches[:])
                new_portal_data[day][emp_id]['exceptions'] = exceptions
                new_portal_data[day][emp_id]['punches'] = clean_punches

                if day_entrance:
                    ex2, cleaned = self._detect_exceptions(day_entrance[:])
                    new_portal_data[day][emp_id]['exceptions'] += ex2
                    new_portal_data[day][emp_id]['entrance_punches'] = cleaned

                # Remove normal entries
                if not new_portal_data[day][emp_id]['exceptions']:
                    new_portal_data[day].pop(emp_id, None)

        # Prepare response
        prev_day = (start_day - timedelta(days=1)).strftime("%Y-%m-%d")
        next_day = (end_day + timedelta(days=1)).strftime("%Y-%m-%d")

        return {
            'portal_data': [list(v.values()) for k, v in new_portal_data.items()],
